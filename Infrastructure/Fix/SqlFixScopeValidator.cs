...
/// <c>{sourceId}</c> → a sentinel token, parses the script with the official
/// T-SQL parser, and requires the sentinel to appear inside the WHERE clause of
/// every UPDATE/DELETE (or inside an EXEC's arguments). String/regex matching is
/// deliberately avoided — only the AST tells us the placeholder is structurally
/// *in the WHERE* rather than merely present somewhere in the text.
/// </summary>
public sealed class SqlFixScopeValidator : ISqlFixScopeValidator
{
    private const string Sentinel = "__MAIA_SRC__";
    private static readonly Regex SourceIdToken = new(@"\{sourceId\}", RegexOptions.IgnoreCase | RegexOptions.Compiled);

    private const string WhereReason =
        "must reference {sourceId} in the WHERE clause to target the specific failure's row. " +
        "Bulk UPDATE/DELETE without per-failure scoping is not allowed.";
    ...
    public string? Validate(string payload)
    {
        ...
        // {sourceId} (any case) → sentinel so the script parses AND the placeholder
        // is locatable in the AST. {failureId} is intentionally NOT substituted.
        var substituted = SourceIdToken.Replace(sql, Sentinel);
            ...
            switch (stmt)
            {
                ...
                case ExecuteStatement e:
                    if (!FragmentHasSentinel(e, tokens))
                        return "EXEC must pass {sourceId} as a parameter to target the specific failure's row.";
                    break;
                default:
